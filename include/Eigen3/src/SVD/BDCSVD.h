...
void BDCSVD<MatrixType>::divide (Index firstCol, Index lastCol, Index firstRowW, Index firstColW, Index shift)
{
    ...
    if (m_compV)
    {
        m_naiveV(firstRowW+k, firstColW) = 1;
    }
    if (r0<considerZero)
    {
        c0 = 1;
...
void BDCSVD<MatrixType>::computeSVDofM(Index firstCol, Index n, MatrixXr& U, VectorType& singVals, MatrixXr& V)
{
    ...
    Index actual_n = n;
    while(actual_n>1 && diag(actual_n-1)==0)
    {
        --actual_n;
    }
    ...
    {
        Index actual_n = n;
        while(actual_n>1 && abs(col0(actual_n-1))<considerZero)
        {
            --actual_n;
        }
        ...
        VectorType& singVals, ArrayRef shifts, ArrayRef mus)
{
    ...
    Index n = col0.size();
    Index actual_n = n;
    while(actual_n>1 && col0(actual_n-1)==0)
    {
        --actual_n;
    }
    ...
    for (Index k = 0; k < n; ++k)
    {
        ...
        while (fCur!=0 && abs(muCur - muPrev) > 8 * NumTraits<RealScalar>::epsilon() * numext::maxi<RealScalar>(abs(muCur), abs(muPrev)) && abs(fCur - fPrev)>NumTraits<RealScalar>::epsilon() && !useBisection)
        {
            ...
            if (shift == left  && (muCur < 0 || muCur > right - left))
            {
                useBisection = true;
            }
            if (shift == right && (muCur < -(right - left) || muCur > 0))
            {
                useBisection = true;
            }
