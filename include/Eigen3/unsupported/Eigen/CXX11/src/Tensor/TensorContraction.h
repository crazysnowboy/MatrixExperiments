...
struct TensorContractionEvaluatorBase
{
          ...
          m_result(NULL), m_expr_indices(op.indices())
    {
        ...
        if (m_i_strides.size() > 0)
        {
            m_i_strides[0] = 1;
        }
        if (m_j_strides.size() > 0)
        {
            m_j_strides[0] = 1;
        }
        if (m_k_strides.size() > 0)
        {
            m_k_strides[0] = 1;
        }
                        ...
                        for (Index mi = mi_outer; mi < mini(mi_outer+mc_outer, m); mi += mc)
                        {
                            ...
                            for (Index ni = ni_outer; ni < mini(ni_outer+nc_outer, n); ni += nc)
                            {
                                ...
                                else
                                {
                                    ...
                                    internal::libxsmm_wrapper<LhsScalar, RhsScalar, Scalar>(0, actual_mc, actual_nc, actual_kc, kernel_stride_A, kernel_stride_B, stride_C, 1, beta, blocking.prefetch())(actual_a, actual_b, c, actual_a,
                                            bp, cp);
                                }
                            }
                        }
