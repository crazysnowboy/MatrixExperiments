...
template <> struct MakeComplex<true>
{
    template <typename T>
    EIGEN_DEVICE_FUNC
    std::complex<T> operator() (const T& val) const
    {
...
{
    template <typename T> T operator() (const std::complex<T>& val) const
    {
        return val.real();
    }
};
...
private:
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void processDataLineBluestein(ComplexScalar* line_buf, Index line_len, Index good_composite, Index log_len, ComplexScalar* a, ComplexScalar* b,
