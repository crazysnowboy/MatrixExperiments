...
public:
    ...
    void operator()(cl::sycl::nd_item<1> itemID)
    {
        ...
        if (globalid< range)
        {
            ...
            Eigen::internal::GenericDimReducer<DeviceSelf::NumReducedDims-1, DeviceSelf, Op>::reduce(device_self_evaluator, device_self_evaluator.firstInput(static_cast<typename DevExpr::Index>(globalid)),
...
public:
    ...
    void operator()(cl::sycl::nd_item<1> itemID)
    {
        ...
        if(globalid<rng)
        {
            tmp_global_accessor.get_pointer()[globalid]=Eigen::internal::InnerMostDimReducer<decltype(device_self_evaluator), Op, false>::reduce(device_self_evaluator,
                    static_cast<typename DevExpr::Index>(red_factor*globalid), red_factor, const_cast<Op&>(op));
        }
        else
        {
        ...
        {
            tmp_global_accessor.get_pointer()[0]+=Eigen::internal::InnerMostDimReducer<decltype(device_self_evaluator), Op, false>::reduce(device_self_evaluator,
                                                  static_cast<typename DevExpr::Index>(red_factor*(rng)), static_cast<typename DevExpr::Index>(remaining), const_cast<Op&>(op));
        }
    }
};
