...
protected:
    ...
    void Getnextvalidmatrix( )
    {
        ...
        while ( (m_curs_id = readdir(m_folder_id)) != NULL)
        {
            ...
            if (m_curs_id->d_type == DT_DIR)
            {
                continue;    //FIXME This may not be available on non BSD systems
            }
//         struct stat st_buf;
...
//         if (S_ISDIR(st_buf.st_mode)) continue;
            ...
            if(!getMarketHeader(curfile,m_sym,iscomplex,isvector))
            {
                continue;
            }
            if(isvector)
