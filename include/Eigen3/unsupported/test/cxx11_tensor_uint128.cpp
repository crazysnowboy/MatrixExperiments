...
void test_misc2()
{
    ...
    for (int64_t log_div = 0; log_div < 63; ++log_div)
    {
        for (int64_t divider = 1; divider <= 1000000 * incr; divider += incr)
        {
            ...
            TensorUInt128<uint64_t, uint64_t> result = (TensorUInt128<uint64_t, static_val<0> >(shift, 0) / TensorUInt128<static_val<0>, uint64_t>(divider) - TensorUInt128<static_val<1>, static_val<0> >(1,
                    0) + TensorUInt128<static_val<0>, static_val<1> >(1));
