...
(
    ...
    const int Ai [ ],          /* row indexes of A, of size nz = Ap [n] */
 ...
 *       Ai: an int/UF_long array of size nz, containing the row indexes of A,
 *           where nz = Ap [n].
 ...
 *
 * On input, the matrix A is stored in column-oriented form.  The row indexes
 * of nonzero entries in column j are stored in Ai [Ap [j] ... Ap [j+1]-1].
 *
 * If the row indexes appear in ascending order in each column, and there
 ...
 *
 * Row indexes must be in the range 0 to
 ...
 * duplicate row indexes in one or more columns.  Returns AMD_INVALID if the
...
(
    ...
    const int Ai [ ]           /* row indexes, of size Ap [n_col] */
) ;
