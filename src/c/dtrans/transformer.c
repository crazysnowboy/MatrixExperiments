    ...
    dtTransformer *trans)
{
    ...
    printf("1-----tclist->list_length = %d ",tclist.list_length);
    __dt_StripTriangleCorrsList(&tclist, n_maxcorrs);


    FILE *fp=fopen("../data/obj/out2.tricorrs","w");
    fprintf(fp,"%d \n",tclist.list_length);
    for (int i_corrs=0 ; i_corrs < tclist.list_length; i_corrs++)
    {
        int source_index= tclist.corr[i_corrs].i_src_triangle;
        int target_index= tclist.corr[i_corrs].i_tgt_triangle;
        fprintf(fp,"%d,%d \n",source_index,target_index);

    }
    fclose(fp);

    printf("2-----tclist->list_length = %d ",tclist.list_length);
    ...
    __dt_CHOLMOD_free_triplet(&A_tri);
    trans->At  = __dt_CHOLMOD_transpose(A);
...
static void __apply_deformation_to_model(dtMeshModel *model, __dt_DenseVector x)
{
    ...
    {
        model->vertex[i].x = __dt_CHOLMOD_REFVEC(x, ind);
        ind++;
        model->vertex[i].y = __dt_CHOLMOD_REFVEC(x, ind);
        ind++;
        model->vertex[i].z = __dt_CHOLMOD_REFVEC(x, ind);
        ind++;
    }
}
